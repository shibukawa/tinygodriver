...
| Petitweb cgosqlite | 2177, 2382, 2251 | 7,507,650 bytes |
...
```sh
go test ./contrib/database/sqlite -run '^$' -bench BenchmarkInsertAndQuery -benchtime=500ms -count=3
go test -tags force_tinygo_logic ./contrib/database/sqlite -run '^$' -bench BenchmarkInsertAndQuery -benchtime=500ms -count=3
CGO_ENABLED=0 go test ./contrib/database/sqlite -run '^$' -bench BenchmarkInsertAndQuery -benchtime=500ms -count=3
```
