...
// RegisterTLSConfig registers a custom TLS configuration to be used with
// sql.Open. Use the key as a value in the DSN where tls=value.
//
// PETITWEB: the parameter is https.Config rather than crypto/tls.Config. TinyGo
// ships a stub crypto/tls whose Client() panics, so TLS goes through the OS
// stack instead; https.Config is that stack's backend-neutral configuration and
// takes PEM bytes, the one representation every backend accepts.
//
...
func RegisterTLSConfig(key string, config *https.Config) error {
	...
	if tlsConfigRegistry == nil {
		tlsConfigRegistry = make(map[string]*https.Config)
	}
...
func getTLSConfigClone(key string) (config *https.Config) {
	tlsConfigLock.RLock()
...
// cloneTLSConfig copies a config so a caller cannot mutate one the driver is
// already using. https.Config has no Clone of its own; the slices are copied
// because the driver hands the result to another goroutine.
func cloneTLSConfig(c *https.Config) *https.Config {
	if c == nil {
		return nil
	}
	cp := *c
	cp.RootCAs = append([][]byte(nil), c.RootCAs...)
	cp.Certificates = append([]https.KeyPair(nil), c.Certificates...)
	return &cp
}
