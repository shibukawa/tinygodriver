...
// tlsHandshakeTimeout bounds the in-band TLS upgrade. DSN timeout= already
// covers connection setup, so reuse it and otherwise fall back to a bounded
// default, because https.Upgrade takes its deadline from a context.
//
// PETITWEB: added for the OS TLS stack; crypto/tls took its deadline from the
// underlying connection instead.
func (mc *mysqlConn) tlsHandshakeTimeout() time.Duration {
	if mc.cfg != nil && mc.cfg.Timeout > 0 {
		return mc.cfg.Timeout
	}
