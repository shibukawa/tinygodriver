...
func (c *connector) Connect(ctx context.Context) (driver.Conn, error) {
	...
	if c.cfg.DialFunc != nil {
		...
		} else if mc.cfg.TLS != nil && mc.cfg.Net == "tcp" {
			// PETITWEB: TLS starts on this very socket after the capability
			// packet, and the OS TLS stacks need the descriptor to do that.
			// TinyGo's net.TCPConn keeps its own private and returns an error
			// from SyscallConn, so the socket has to come from https.DialPlain,
			// whose connection carries one.
			var host, port string
			if host, port, err = net.SplitHostPort(mc.cfg.Addr); err == nil {
				mc.netConn, err = https.DialPlain(dctx, host, port)
			}
		} else {
			nd := net.Dialer{}
