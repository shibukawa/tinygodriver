...
func (mc *mysqlConn) writeHandshakeResponsePacket(authResp []byte, plugin string) error {
	...
	if mc.cfg.TLS != nil {
		...
		// PETITWEB: the handshake goes through the OS TLS stack instead of
		// crypto/tls, whose TinyGo stub panics. https.Upgrade needs a
		// connection carrying its descriptor, which connector.go arranges with
		// https.DialPlain whenever cfg.TLS is set; a caller-supplied DialFunc
		// gets https.ErrNotUpgradable rather than a silent cleartext session.
		host, _, err := net.SplitHostPort(mc.cfg.Addr)
		if err != nil {
			host = mc.cfg.Addr
		}
		ctx, cancel := context.WithTimeout(context.Background(), mc.tlsHandshakeTimeout())
		defer cancel()
		tlsConn, err := https.Upgrade(ctx, mc.netConn, host, mc.cfg.TLS)
		if err != nil {
			if cerr := mc.canceled.Value(); cerr != nil {
				return cerr
			}
