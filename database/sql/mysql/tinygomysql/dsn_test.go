...
func TestDSNWithCustomTLSQueryEscape(t *testing.T) {
	...
	name := "foohost"
	tlsCfg := https.Config{ServerName: name}
...
func TestNormalizeTLSConfig(t *testing.T) {
	tt := []struct {
		...
		{"", nil},
		{"false", nil},
		{"true", &https.Config{ServerName: "myserver"}},
		{"skip-verify", &https.Config{InsecureSkipVerify: true}},
		{"preferred", &https.Config{InsecureSkipVerify: true}},
		{"test_tls_config", &https.Config{ServerName: "myServerName"}},
	}

	RegisterTLSConfig("test_tls_config", &https.Config{ServerName: "myServerName"})
